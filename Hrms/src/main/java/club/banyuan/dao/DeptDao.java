...
=======
    ...
    @Insert("insert into department (name,description) values(#{name},#{description})")
     int addDept(@Param("name")String name,@Param("description") String description);
    @Select("select *from department where name=#{name}")
    @ResultType(Dept.class)
     Dept getDeptByName(String name);
     int deleteDeptById(List<Integer> idList);
    @Update("update department set name=#{name},description=#{description} where id=#{id}")
     int updateDeptById(@Param("name")String name,@Param("description") String description,@Param("id") Integer id);
    @Select("select *from department where name=#{name} and id!=#{id}")
    @ResultType(Dept.class)
     Dept getDeptByNameAndId(@Param("name") String name,@Param("id") Integer id);
>>>>>>> fc5f787... 1
}
